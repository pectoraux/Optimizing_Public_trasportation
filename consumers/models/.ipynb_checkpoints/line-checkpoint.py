...
STATIONS_TOPIC = "org.chicago.cta.station"
...
class Line:
    ...
    def process_message(self, message):
        ...
        if STATIONS_TOPIC in message.topic(): # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif ARRIVAL_TOPIC in message.topic(): # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif TURNSTILE_SUMMARY_TOPIC == message.topic(): # Set the conditional to the KSQL Turnstile Summary Topic
